...
def generate_data():
    ...
    if is_even_week():
        if datetime.date.today().weekday() == 0:
            ...
            #             t = [row[cell].value, row[cell + 1].value]
            #             data.append(t)
            data.append(["8:10 - 18:40", "ЧИИИЛ"])
    else:
        if datetime.date.today().weekday() == 0:
            for row in sheet.iter_rows(min_row=2, max_row=6, min_col=5, max_col=7):
                for cell in range(0, 3, len(row)):
                    if row[cell + 2].value:
                        t = [row[cell].value, row[cell + 2].value]
                        data.append(t)
        elif datetime.date.today().weekday() == 1:
            for row in sheet.iter_rows(min_row=7, max_row=11, min_col=5, max_col=7):
                for cell in range(0, 3, len(row)):
                    if row[cell + 2].value:
                        t = [row[cell].value, row[cell + 2].value]
                        data.append(t)
            ...
            for row in sheet.iter_rows(min_row=2, max_row=6, min_col=10, max_col=12):
                for cell in range(0, 3, len(row)):
                    if row[cell + 2].value:
                        t = [row[cell].value, row[cell + 2].value]
                        data.append(t)
        elif datetime.date.today().weekday() == 4:
            for row in sheet.iter_rows(min_row=7, max_row=11, min_col=10, max_col=12):
                for cell in range(0, 3, len(row)):
                    if row[cell + 2].value:
                        t = [row[cell].value, row[cell + 2].value]
                        data.append(t)
